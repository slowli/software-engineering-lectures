...
\begin{document}
	...
	\frame{
		...
		\vspace{0.5ex}
		\textbf{Python:}
		\lstinputlisting[language=python,escapechar=\$]{code-str-int.py}

		\vspace{0.5ex}
		\textbf{PHP:}
		\lstinputlisting[language=php]{code-str-int.php}
	...
	\frame{
		...
		\begin{enumerate}
			\item
			Шаблоны — автоматическая генерация мало различающихся параметризованных фрагментов кода. 
			...
			\vspace{1ex}\item
			Интерпретация произвольного исходного кода с помощью специальных функций.

			\vspace{0.5ex}
			\textbf{Примеры:} Функция \code{eval} в Python, PHP, JavaScript и других интерпретируемых ЯП.

			\vspace{1ex}\item
			Использование рефлексии \engterm{reflection} для самомодификации программы во~время выполнения.

			\vspace{1ex}\item
			Использование предметно-специфичных языков \engterm{domain-specific languages} 
	...
	\frame{
		...
		\lstinputlisting[language=python,escapechar=\$]{code-view.py}
	}
	...
	\frame{
		...
		\begin{itemize}
			\item
			Затраты на проектирование, имплементацию и сопровождение языка и~соответствующих инструментов 
