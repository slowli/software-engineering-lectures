...
\begin{document}
	...
	\frame{
		...
		\lstinputlisting[language=python,firstline=4]{code-duck.py}
	}
	...
	\frame{
		...
		\begin{overlayarea}{\textwidth}{0.575\textheight}
			\only<1,2>{%
				...
				\lstinputlisting[language=java,escapechar=\#]{code-wildcards-1.java}
			}
			\only<3,4>{%
				\textbf{Пример (контравариантное определение):}
				\lstinputlisting[language=java,escapechar=\#]{code-wildcards-2.java}
			}
		\end{overlayarea}
	}
